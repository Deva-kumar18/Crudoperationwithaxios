    ...
    <div className="table">
      <div className="inputs">
        ...
        {isEditing ? (
          ...
          <button  className="add-btn" onClick={createUser}><img className="add-ico" src={ADD}/></button>
        )}
      </div>
      <table>
        <thead>
        ...
        <tbody>
          {users.map((user) => (
            <tr key={user.id}>
              ...
              <td>{user.email}</td>
              <td>
                <button className="inputs-2" onClick={() => deleteUser(user.id)}><img className="del-ico" src={Delete}/></button>
                <button className="inputs-2"
                  onClick={() => editUser(user.id, user.username, user.email)}
                >
                  <img  className="edit-ico" src={Edit}/>
                </button>
              </td>
            </tr>
          ))}
